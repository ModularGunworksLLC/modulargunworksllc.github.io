...
| Invoice amount validation | `/api/guntab-create-invoice` rejects mismatched merchandise totals (allow ±1 cent) |  |
| Shipping amount validation | `/api/guntab-create-invoice` rejects shipping that does not match threshold rule |  |
| Trusted invoice payload | Endpoint sends trusted merchandise/shipping cents to GunTab |  |
