...
This repository is the **version-controlled source** for the **custom WordPress themes** used on **AWS Lightsail** (MGW `mgw-*` plugins were removed from `wordpress-package/` — keep them on the server or restore from Git history). The live storefront is **[modulargunworks.com](https://www.modulargunworks.com)**.
...
| **`wordpress-package/plugins/`** | **Empty placeholder** — former `mgw-*` plugins are not versioned here. |
