...
// LOAD JSON
// ======================================================
async function loadReloading() {
  const response = await fetch("../Data/reloading-data.json");
...
function getStockImage(item) {
  // Map reloading types to relevant stock image keywords
  const imageMap = {
    "Press": "reloading-press,ammunition-press",
    "Scale": "powder-scale,reloading-scale",
    "Dies": "reloading-dies,die-set"
  };

  const type = item.type;
  const keyword = imageMap[type] || "reloading-equipment,ammunition-reloading";
    ...
    <article class="ammo-card ${inStock ? "" : "ammo-out-of-stock"}">
      ...
      <div class="ammo-card-body">
        ...
        <ul class="ammo-card-specs">
          ...
          ${item.calibers ? `<li><strong>Calibers:</strong> ${item.calibers}</li>` : ''}
          ${item.accuracy ? `<li><strong>Accuracy:</strong> ${item.accuracy}</li>` : ''}
