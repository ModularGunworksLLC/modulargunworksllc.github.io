...
define('MGW_CHATTANOOGA_SYNC_VERSION', '1.0.1');
define('MGW_CHATTANOOGA_SYNC_PATH', plugin_dir_path(__FILE__));
define('MGW_CHATTANOOGA_SYNC_URL', plugin_dir_url(__FILE__));
define('MGW_CHATTANOOGA_OPTION_SCHEDULED_SYNC', 'mgw_chattanooga_scheduled_sync_enabled');
...
/**
 * Clear WP-Cron sync unless explicitly enabled (manual "Sync Now" always works).
 */
function mgw_chattanooga_sync_maybe_clear_disabled_cron() {
    if (!class_exists('WooCommerce')) {
        return;
    }
    if (get_option(MGW_CHATTANOOGA_OPTION_SCHEDULED_SYNC, '0') !== '1') {
        wp_clear_scheduled_hook('mgw_chattanooga_sync_cron');
    }
}
add_action('plugins_loaded', 'mgw_chattanooga_sync_maybe_clear_disabled_cron', 19);
...
add_action('plugins_loaded', 'mgw_chattanooga_sync_init', 20);

/**
 * Activation: do not schedule cron until "Enable scheduled sync" is checked in settings.
 */
